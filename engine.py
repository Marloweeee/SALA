...
def train_one_epoch(args, model: torch.nn.Module, txt_enc_assistant, data_loader: Iterable, 
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        ...
        img_data, text_data, target,raw_queries, img_features = batch
        ...
        img_teacher_features=img_features # [B,768]
        txt_teacher_features = txt_enc_assistant.encode(
                raw_queries, device=device, show_progress_bar=False, convert_to_tensor=True).to(device, non_blocking=True)
        ...
        with autocast(enabled=args.amp):
            ...
            inter_loss, intra_loss=loss_utils.SALA_Loss(text_cls,visu_src,visu_token_factor,txt_teacher_features,img_teacher_features,args)
            loss_dict['sa_loss'],loss_dict['la_loss']=args.inter_weight*inter_loss,args.intra_weight*intra_loss
