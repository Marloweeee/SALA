...
class TransVGDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', 
        ...
        self.split = split
        self.tokenizer = BertTokenizer.from_pretrained('/home/u2023010354/code/QRNet-BUPT/checkpoints/bert_base_uncased', do_lower_case=True)
        ...
        self.unicom_features=np.load(unicom_path, allow_pickle=True).item()
    ...
    def pull_item(self, idx):
        ...
        if self.dataset == 'flickr':
            ...
            img_features=self.unicom_features[img_file+unique_flag]
        ...
        bbox = bbox.float()
        return img, phrase, bbox, img_features
    ...
    def __getitem__(self, idx):
        ...
        img, phrase, bbox, img_features = self.pull_item(idx)
        # phrase = phrase.decode("utf-8").encode().lower()
        phrase = phrase.lower()
        input_dict = {'img': img, 'box': bbox, 'text': phrase, 'img_features': img_features}
        ...
        if self.testmode:
            ...
            if self.split=='train':
                 return img, np.array(img_mask), np.array(word_id, dtype=int), np.array(word_mask, dtype=int), np.array(bbox, dtype=np.float32), phrase, np.array(img_features, dtype=np.float32)
            else:
                return img, np.array(img_mask), np.array(word_id, dtype=int), np.array(word_mask, dtype=int), np.array(bbox, dtype=np.float32), phrase, np.array(img_features, dtype=np.float32)